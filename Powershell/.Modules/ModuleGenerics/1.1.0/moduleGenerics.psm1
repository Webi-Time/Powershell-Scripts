...
<#
    ...
    .EXAMPLE
        Log "Script" "This is a log message." 1 Cyan

    .EXAMPLE
        Log "Script","Action" "Succesffuly updated." 1 Green
...
function Write-JobLog {
    param (
        $ParentJobLog,
        [switch]$RemoveChildLog=$false
    )

    $ParentLog = Get-ChildItem -Path $ParentJobLog -Filter '*.log' | where { $_.Name -like "*$global:Date_Logs_File*"}
    $ChildsLog = @()
    $ChildsLogFolder = Get-ChildItem -Path $ParentJobLog -Attributes Directory
    #
    #(Get-ChildItem -Path $ParentJobLog -Attributes Directory).FullName
    #(Get-ChildItem -Path $ParentJobLog -Recurse -Attributes Directory).FullName

    foreach ($ChildLogFolder in $ChildsLogFolder) {
        $ChildsLog += Get-ChildItem -Path $ChildLogFolder.FullName -Recurse -Filter '*.log' | where { $_.Name -like "*$global:Date_Logs_File*"}
    }
    
   foreach ($log in $ChildsLog) {
        Get-Content $log.FullName -Raw | Out-File $ParentLog.FullName -Append -Force
        if($RemoveChildLog){
            Remove-Item $log.DirectoryName -Recurse -Force
        }
   }
...
$Global:previousTime = get-date
function Show-ProgressLibrary($Items,$maxItems,$RefreshInterval = 30, $texte="Processing...") 
        {
            if($Items -eq $maxItems){
                Write-Progress -Activity "End Process" -Status "Finish" -SecondsRemaining 0 -PercentComplete 100 -Completed
                return 0
            }
...
function Receive-AllJob  {
    ...
    [PSObject[]]$ResultJobs = $null
    [int]$nbJobsTotal=0
    [int]$nbJobsFini=0
    $nbJobs = (Get-Job | Where {$_.Name -like "$JobNames*"}).count
    $nbJobsTotal = $nbJobs

    $StartJob = Get-Date
    Log "Script" "Starting [$nbJobs] jobs - $JobNames" 2 Green

    while ((Get-Job -State Running | Where {$_.Name -like "$JobNames*"}) -or (Get-Job -State Failed | Where {$_.Name -like "$JobNames*"}) -or (Get-Job -State Completed | Where {$_.Name -like "$JobNames*"})) {
        Start-Sleep -Seconds $waitTime
        Log "Script" "$(Get-Job |select Name,State) " 1 Green
        $jobs = $null
        $lesJobs = Get-job | Where-Object {$_.Name -like "$JobNames*" }            
        $jobs = $lesJobs | Where-Object {($_.State -eq "Completed" -or $_.State -eq "Failed")}


        $jobs | %{$_|Show-JobProgress}
        $nbJobs = $lesJobs.count
        if (-not ([string]::IsNullOrEmpty($jobs)))
        {
            foreach ($job in $jobs)
            {
                try {
                    $ResultJobs += $(Receive-Job $job)  
                }
                catch {
                    Get-DebugError $_
                ...
                Remove-Job $job
                $nbJobsFini++
                #Log "Script" "$($job.Name) est exploité" 1 Green
            }

        }
        #$EndScript = Get-Date
        #$Temps = ((Get-Date $EndScript)-(Get-Date $StartJob)).toString().Split('.')[0]
        #Log "Script" "$($Temps.Split(':')[0])h$($Temps.Split(':')[1])m$($Temps.Split(':')[2])s - Attente des jobs (Fini : $($nbJobsFini) | En cours : $($nbJobsTotal - $nbJobsFini))..." 1 Magenta
...
Function InitVarType(){
    param (
        ...
        $Value, 
        [Parameter(Mandatory = $false)]
        [ValidateSet("Global","Local","Script","Private","SuperGlobal")]
        $Scope = "SuperGlobal"
    )
    ...
    if ($Scope -eq "SuperGlobal"){
        Set-Variable $VariableName -Scope $Scope -Value ($Value -as ($Type -as [type])) -Option AllScope
    }else{
        Set-Variable $VariableName -Scope $Scope -Value ($Value -as ($Type -as [type]))
    }
    #log "Script" " $((Invoke-Expression "`$$($VariableName)").gettype()) | -Type $outType -Value $InitialValue -Scope $scope"
   # Write-Output $((Invoke-Expression "`$$($VariableName)"))
...
function ClearVar {
    ...
    foreach ($var in $vars) {
        try{
            Log "Script" "Remove Var [$var]" 3 DarkMagenta
...
function WriteJobProgress
{
    ...
    #Write-Output ($Job |ConvertTo-Json -Depth 3 | Out-String)
    ...
    { 
        ...
        if ($child) {
             $JobHistoryGroup = $jobProgressHistory | Group-Object ActivityId | Select-Object @{l="ActivityId";e={$_.Name}},Count,group
        }else {
            $JobHistoryGroup = $jobProgressHistory | where {$_.ParentActivityId -eq -1} | Group-Object ActivityId | Select-Object @{l="ActivityId";e={$_.Name}},Count,group
        }
        foreach ($progressBar in $JobHistoryGroup) {
            $progressBar = $progressBar.Group[-1]
            $latestPercentComplete = $progressBar | Select-Object -expand PercentComplete;
            $latestActivity = $progressBar | Select-Object -expand Activity;
            $latestStatus = $progressBar | Select-Object -expand StatusDescription;
            #Write-host "$($progressBar.ActivityId) | $($progressBar.ParentActivityId) | $latestActivity | $latestStatus | $progressBar | $latestPercentComplete" -f yellow
            ...
            if ([string]::IsNullOrEmpty($job.ParentId)){
                Write-Progress -Id $progressBar.ActivityId -ParentId $($progressBar.ParentActivityId) -Activity $latestActivity -Status $latestStatus -PercentComplete $latestPercentComplete;
            }else {
                Write-Progress -Id $progressBar.ActivityId -ParentId $($progressBar.ParentActivityId) -Activity $latestActivity -Status $latestStatus -PercentComplete $latestPercentComplete;
            }
        }
...
function waitJobsPlease ($JobsPrefixName) {
    ...
    [int]$nbJobsTotal=0
    [int]$nbJobsFini=0
    $nbJobs = (Get-Job | Where-Object {$_.Name -like "$JobsPrefixName*"}).count
    $nbJobsTotal = $nbJobs
    Write-Host "I have launched $nbJobs jobs" -f Cyan

    $StartJob = Get-Date
    [int]$seconde = 2
    $Global:previousTime= Get-Date
    while ((Get-Job -State Running| Where {$_.Name -like "$JobsPrefixName*"}) -or (Get-Job -State Completed| Where {$_.Name -like "$JobsPrefixName*"}) -or (Get-Job -State Failed| Where {$_.Name -like "$JobsPrefixName*"})) 
    {
        ...
        $lesJobs = Get-job | Where-Object {$_.Name -like "$JobsPrefixName*" }
        $jobs = $lesJobs | Where-Object {($_.State -eq "Completed" -or $_.State -eq "Failed")}
        $JobRunning = $lesJobs | Where-Object {$_.State -eq "Running"}            
        $JobRunning | ForEach-Object {WriteJobProgress $_ | Out-Null}

        if (-not ([string]::IsNullOrEmpty($jobs)))
        {
            foreach ($job in $jobs)
            {
                Write-host $job.Name -f green
                $result = Receive-Job $job                             
                WriteJobProgress $job | Out-Null                            
                Remove-Job -id $job.Id             
                if ($result) {
                    $ChildJobsResults += $result
                }                           
                $nbJobsFini++                           
            }
        }
