...
function Get-FirstAlphabeticLetter {
    ...
    if ($Global:Inputstring -eq $null -or $Global:Inputstring.Length -eq 0) {
    ...
    if ($firstAlphabeticLetter -eq $null) {
...
function Format-Name($name) {
    ...
    if ($groupName -match '\[([^[\]]+)\]') {
        ...
        if ([string]::IsNullOrEmpty($Global:Script_Inputs_Hashtable)) {
            ...
            Log "Script" "Actual Inputs : `r`n$($Global:Script_Inputs_Hashtable.keys | ft | Out-String)" 1 Red -NoDate
...
function Get-XMLInfo ($node,[hashtable]$Output,$DomainDN) {
   ...
   try {
        switch ( $node.LocalName ) {
                ...
                $RootPath = Format-Name $DomainDN 
                $RootDesc = Format-Name $node.Description



                $cano = $(Get-Canonical $node) -Replace "/Root", $DomainDN.Replace(",DC=",".").Replace("DC=","")
                $OUItem = New-Object PSObject    
                $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "OUName"        -Value $RootName
                $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "OUPath"        -Value $RootPath
                $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "CanonicalName"  -Value $cano
                $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "OUDescription" -Value $RootDesc          
                $OUItem | Add-Member -MemberType NoteProperty -TypeName PSObject -Name "OUDelegation"  -Value $null           
            ...
            "OrgUnit" {  
                    ...
                    $OUPath = Format-Name $($path.Replace("OU=Root,", $DomainDN))
                    $OUDesc = Format-Name $node.Description               
              
                    $OUCano = $(Get-Canonical $node) -Replace"/Root", $DomainDN.Replace(",DC=",".").Replace("DC=","")

                    $OUItem = New-Object PSObject    
                    $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "OUName"        -Value $OUName
                    $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "OUPath"        -Value $OUPath
                    $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "CanonicalName" -Value $OUCano
                    $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "OUDescription" -Value $OUDesc           
                    $OUItem | Add-Member -MemberType NoteProperty -TypeName PSObject -Name "OUDelegation"  -Value $null           
                    $OUItem | Add-Member -MemberType NoteProperty -TypeName PSObject -Name "Groups"        -Value $null

                
                try {
                    $t= Get-ADOrganizationalUnit -Identity ("OU=" + $OUName + "," + $OUPath) -ErrorAction Stop | Out-Null
                ...
                Log "Debug" "OrgUnit = $($OUItem | select * | ft -AutoSize | Out-String)" 5 Cyan 
            }
            "Delegation" {
                $delGroupName = $(Format-Name $node.DelegateGroup)
                $delGroupPath = $(Format-Name $node.DelegateGroupPath)
                if (-not [string]::IsNullOrEmpty($delGroupPathh)) {
                    $delGroupPath += ("," + $DomainDN)
                }
                ...
                if ([string]::IsNullOrEmpty($node.Identity)) { 
                    try {
                        $DelGroup = Get-ADGroup -Identity $delGroupName
                        Log "Script" "[Group Task] - $($delGroupName) exist" 3 darkGreen
                        $DelegationOU | Add-Member -MemberType NoteProperty -TypeName bool -Name "DelegateGroupExist" -Value $true
                    }
                    catch {
                        Log "Script" "[Group Task] - $($delGroupName) didn't exist" 3 DarkYellow
                        if ([string]::IsNullOrEmpty($node.Identity)) { 
                            $DelegationOU | Add-Member -MemberType NoteProperty -TypeName bool -Name "DelegateGroupExist" -Value $false
                        }else{
                            $DelegationOU | Add-Member -MemberType NoteProperty -TypeName bool -Name "DelegateGroupExist" -Value $true
                        }
                    }  
            ...
            else
            {
                foreach ($group in $(($node.Roles).split(','))) 
                {
                    ...
                    if (-not [string]::IsNullOrEmpty($RoleGroupPath)) {
                        $RoleGroupPath += ("," + $DomainDN)
                    }
                    $RoleGroupPath = $(Format-Name $RoleGroupPath)

                    try     {$rg = Get-ADGroup -Identity $($RoleGroupName); Log "Script" "[Group Role] - $($RoleGroupName) exist" 3 DarkGreen}
                    catch   {Log "Script" "[Group Role] - $($RoleGroupName) didn't exist" 3 DarkYellow}
                    finally {
                        ...
                        $RoleGroup | Add-Member -MemberType NoteProperty -TypeName bool -Name "RoleGroupExist" -Value $($null -ne $rg)
                        $rolesG += $roleGroup
                    }
                }
            }
            ...
            if ($null -eq $Output.$key.OUDelegation) {
                $Output.$key.OUDelegation = @()
            }
            Log "Script" "Addind delegation rule to OU $key" 3 DarkGray
            $Output.$key.OUDelegation += $DelegationOU
            ...
            "Group" {
                ...
                $GroupPath = "OU=" + $(Format-Name $Output.$key.OUName)+ "," + $(Format-Name $Output.$key.OUPath)
                $GroupPath = $(Format-Name $GroupPath)

                $ObjGroup= New-Object PSObject                   
                $ObjGroup | Add-Member -MemberType NoteProperty -TypeName string -Name "GroupName"  -Value $GroupName
                $ObjGroup | Add-Member -MemberType NoteProperty -TypeName string -Name "GroupDesc"  -Value $GroupDesc
                $ObjGroup | Add-Member -MemberType NoteProperty -TypeName string -Name "GroupPath"  -Value $GroupPath
                $ObjGroup | Add-Member -MemberType NoteProperty -TypeName string -Name "GroupScope" -Value $node.Scope
                [PSObject]$rolesG = @()
                if ([string]::IsNullOrEmpty($node.Members)) 
                {
                    $rolesG = $null
                }
                else
                {
                    foreach ($group in $(($node.Members).split(','))) 
                    {
                        $MemberGroup= New-Object PSObject
                        $MemberGroupName = $(Format-Name $group)
                        $MemberGroupPath = $(Format-Name $node.MembersPath)
                        $MemberGroupDesc = $(Format-Name $node.MembersPath)
                        if (-not [string]::IsNullOrEmpty($MemberGroupPath)) {
                            $MemberGroupPath += ("," + $DomainDN)
                        }
                        
                        $MemberGroupPath = $(Format-Name $MemberGroupPath)
                        try     {$rg = Get-ADGroup -Identity $( $MemberGroupName); Log "Script" "[Group Others] - $( $MemberGroupName) exist" 3 DarkGreen}
                        catch   {Log "Script" "[Group Others] - $( $MemberGroupName) didn't exist" 3 DarkYellow}
                        finally {
                            $MemberGroup | Add-Member -MemberType NoteProperty -TypeName string -Name "MembersName"   -Value $MemberGroupName
                            $MemberGroup | Add-Member -MemberType NoteProperty -TypeName string -Name "MembersDesc"   -Value $MemberGroupDesc
                            ...
                            $MemberGroup | Add-Member -MemberType NoteProperty -TypeName bool -Name "MembersGroupExist" -Value $($null -ne $rg)
                            $rolesG += $MemberGroup
                        }
                    }
                }
                $ObjGroup | Add-Member -MemberType NoteProperty -TypeName PSObject -Name "Members" -Value $rolesG
...
Function Test-OUs
{
    Param( $OUName, $OUPath, $Description, [bool]$Protected = $true ,$Server = $Global:RessourceDomain_DNSName,[switch]$whatIf = $false )
    ...
    try {
        If (([adsi]::Exists("LDAP://OU=$OUName,$OUPath" )) -Eq $false ) 
        {
            if (-not $whatIf) {
                ...
                New-ADOrganizationalUnit -Server $Server -Name $OUName -Path $OUPath -Description $Description -ProtectedFromAccidentalDeletion $Protected 
...
Function Test-ADGroups
{
    ...
    try{
        ...
        {
            if ([string]::IsNullOrEmpty($(try{($gg = Get-ADGroup -Server $Server -Identity $GroupName -ErrorAction SilentlyContinue)}catch{$null})))
            {
                ...
                if (-not $whatIf) {
                    if ($GroupDisplayName){
                        New-ADGroup -Server $Server -DisplayName $GroupDisplayName -name $GroupName -GroupCategory Security -GroupScope $groupScope -Path $GroupPath -Description $GroupDescription 
                    }else{
                        New-ADGroup -Server $Server -name $GroupName -GroupCategory Security -GroupScope $groupScope -Path $GroupPath -Description $GroupDescription 
                    }
                    Get-ADGroup -Server $Server -Identity $GroupName  | Set-ADObject -Server $Server -ProtectedFromAccidentalDeletion $Protected
                    
                    Log "Script" "Creation du groupe $GroupName - [$GroupPath] - [$groupScope]" 1 Cyan
                }else{
                    Log "Script" "Whatif - Creation du groupe $GroupName - [$GroupPath] - [$groupScope]" 1 Gray
                }  
            }else{
                Log "Script" "Le group exite mais dans une autre OU $GroupName [$($gg.DistinguishedName)]" 1 yellow
                return 0
            }        
        }else{
            Log "Script" "Le groupe existe deja $GroupName - [$GroupPath]" 1 Green
            return 0
        }
        return 1
    }catch {
        log "Error" "Error in : LDAP://CN=$GroupName,$GroupPath" 1 red
        Get-DebugError $_
        return -1
    }
...
function Set-ADDelegation()
{
    ...
    if ($Server -ne $CurrentServer){
        ...
        $ad = "AD2:"
        if ([string]::IsNullOrEmpty($(Get-PSDrive -Name AD2 -ErrorAction SilentlyContinue))){
