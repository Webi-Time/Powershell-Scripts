   ...
   try {
        switch ( $node.LocalName ) {
            ...
            "OrgUnit" {  
                    ...
                    $OUItem | Add-Member -MemberType NoteProperty -TypeName string   -Name "OnlyAdm"        -Value $node.OnlyAdm
            ...
            "Group" {
                ...
                if([string]::IsNullOrEmpty($node.Path)){
                    ...
                    $GroupPath = $(Format-Name $node.Path)
                    if ($node.Path -notlike "*DC=*") {
...
function Get-Canonical([string]$OUPath,[string]$OUName) {
...
function Get-FormatOU ($OUName,$OUPath,$OUDescription=""){
        ...
        $Canonical = Get-Canonical -OUPath $OUPath -OUName $OUName
